...
2. Add `seedance2.0 @ picker helper`.
3. Connect the same reference images into both places:
   ...
   - `seedance2.0 @ picker helper` `Resources_0`, `Resources_1`, etc., so the picker can show thumbnails.
4. Connect `seedance2.0 @ picker helper` `prompt` output to the `Seedance2DPE` prompt input.
5. Type your prompt in `seedance2.0 @ picker helper`.
...
Keep the image order the same between `Seedance2DResource` and `seedance2.0 @ picker helper`. For example, if the first image connected to `Seedance2DResource` is the character image, connect that same image to `seedance2.0 @ picker helper` `Resources_0`.
